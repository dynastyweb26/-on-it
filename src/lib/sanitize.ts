...
export function sanitizeForAI(raw: string): string {
  return raw
    ...
    // Strip tag-like sequences (including those with attributes or trailing slashes)
    // to prevent prompt injection breakouts (e.g. <user_input attr="val"> or </user_input>).
    .replace(/<\/?[a-z_:-][^>]*>/gi, ' ')
