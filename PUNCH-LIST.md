...
| Unmerged work to port | **Open** | Not yet merged to `main`: date dividers + deductible removal, loading skeletons, PDF fonts + PNG. Soft-delete + back nav (Jules `2ef8490`) are ported on branch `feat/soft-delete-backnav` (pending merge); the swipe threshold/velocity tuning from `2ef8490` is **dropped** — not ported by decision. Reported 2026-09-15; confirm each branch before porting. |
