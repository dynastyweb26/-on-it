...
| hardcoded invoice language (greeting/CTA/"ready to send") | **Partial** | Greeting **still hardcodes** "invoice": `chat/page.tsx:85` ("…I'll handle the invoice."). Fixed elsewhere: send CTA is neutral — `chat/page.tsx:1549` "Looks right — send it" (`de407ee`); confirm summary and the "…is sent" line are kind-aware via `${kind}` (`chat/page.tsx:1020`, `confirmSummary`); the preview card title is kind-aware (`chat/page.tsx:1540`). The download message `chat/page.tsx:1098` is neutral (names no kind). |
